...
async function scanOnce(baseUrl) {
  ...
  for (const session of sessions) {
    ...
    for (const entry of completedAssistantMessages) {
      ...
      if (!messageId || !completedAt || state.seen[messageId]) {
        continue;
      }
...
async function extractTelegramAttachments(message) {
  ...
  for (const item of items) {
    if (Number(item.size || 0) > ATTACHMENT_SIZE_LIMIT) {
      notes.push(`Attachment skipped because it exceeds ${ATTACHMENT_SIZE_LIMIT} bytes: ${item.filename}`);
      continue;
    }
...
function topicNameForSession(sessionId, title) {
  ...
  const clean = String(title || "Untitled session")
    .replace(/\s+/g, " ")
    .trim();

  return truncate(`[${shortId}] ${clean || "Untitled session"}`, 120);
}
...
function getDesktopNotifyArgs(item) {
  const title = `OpenCode: ${truncate(item.title || "Untitled session", 60)}`;
